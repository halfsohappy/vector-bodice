"""CLI for the culotte pattern.

Measurement arguments come from manifest.json (the same source the web
frontend uses).

    python -m patterns.culotte --waist_arc_front 7 --waist_arc_back 7 \
        --hip_arc_front 9.5 --hip_arc_back 9.5 \
        --hip_depth_front 9 --hip_depth_back 9 --crotch_depth 10.5 \
        --skirt_length 24
"""
...
parser = argparse.ArgumentParser(
    prog="python -m patterns.culotte",
...
parser.add_argument("--prefix", type=str, default="culotte",
...
render(vals["waist_arc_front"], vals["waist_arc_back"],
       ...
       vals["hip_depth_front"], vals["hip_depth_back"],
       vals["crotch_depth"], vals["skirt_length"],
       prefix=args.prefix, seam_allowance=args.seam_allowance,
       **collect_options(args, OPTIONS))
